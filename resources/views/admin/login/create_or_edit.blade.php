            ...
            <form class="layui-form" action="" lay-filter="currentForm" onsubmit="return false;">
                <div class="layui-tab layui-tab-brief">
                    ...
                    <div class="fr page-a-link">
                        ...
                        @if(isset($login->id))
                          <a class="btn-link" href="{{url('admin/login/create')}}">前往添加</a>
                          <a class="btn-link" href="{{url('admin/login/'.$login->id)}}">前往查看</a>
                          @else
                          <a class="btn-link" href="{{url('admin/login')}}">返回列表</a>
                        @endif
                        </span>
                    </div>
    ...
    <script type="text/javascript">
        layui.use(['element', 'form', 'jquery', 'layedit', 'laydate', 'systemGui'], function () {
            ...
            form.on('submit(formPOST)', function (data) {
                systemGui.createOrUpdate(data.field, 'POST','{{url('admin/login')}}')
                return false;
            });
            //更新
            form.on('submit(formPUT)', function (data) {
                systemGui.createOrUpdate(data.field, 'PUT','{{url('admin/login',$login->id)}}')
