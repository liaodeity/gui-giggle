                ...
                <div class="layui-tab layui-tab-brief">
                    ...
                    <div class="fr page-a-link">
                        ...
                        @if(isset($block->id))
                          ...
                          <a class="btn-link" href="{{url('admin/block/'.$block->id)}}">{{__('message.tab.go_view')}}</a>
                          @else
                          <a class="btn-link" href="{{url('admin/block')}}">{{__('message.tab.back_list')}}</a>
                        @endif
                        </span>
                    </div>
                    ...
                    <div class="layui-input-block">
                        <input type="text" name="Block[module_id]" value="{{$block->module_id ?? ''}}" maxlength="36" autocomplete="off"
                               placeholder=""
                               class="layui-input">
                    </div>
                </div>
<div class="layui-form-item">
                    <label class="layui-form-label">视图块名称 <span class="color-red"></span></label>
                    <div class="layui-input-block">
                        <input type="text" name="Block[name]" value="{{$block->name ?? ''}}" maxlength="191" autocomplete="off"
                               placeholder=""
                               class="layui-input">
                    </div>
                </div>
    ...
    <script type="text/javascript">
        layui.use(['element', 'form', 'jquery', 'layedit', 'laydate', 'systemGui'], function () {
            ...
            form.on('submit(formPOST)', function (data) {
                systemGui.createOrUpdate(data.field, 'POST','{{url('admin/block')}}')
                return false;
            });
            //更新
            form.on('submit(formPUT)', function (data) {
                systemGui.createOrUpdate(data.field, 'PUT','{{url('admin/block',$block->id)}}')
