                ...
                <div class="layui-tab layui-tab-brief">
                    ...
                    <div class="fr page-a-link">
                        ...
                        @if(isset($tab->id))
                          ...
                          <a class="btn-link" href="{{url('admin/tab/'.$tab->id)}}">{{__('message.tab.go_view')}}</a>
                          @else
                          <a class="btn-link" href="{{url('admin/tab')}}">{{__('message.tab.back_list')}}</a>
                        @endif
                        </span>
                    </div>
                    ...
                    <label class="layui-form-label">表名称 <span class="color-red"></span></label>
                    <div class="layui-input-block">
                        <input type="text" name="Tab[name]" value="{{$tab->name ?? ''}}" maxlength="191" autocomplete="off"
                               placeholder=""
                               class="layui-input">
                    </div>
                </div>
<div class="layui-form-item">
                    <label class="layui-form-label">表标题 <span class="color-red"></span></label>
                    <div class="layui-input-block">
                        <input type="text" name="Tab[title]" value="{{$tab->title ?? ''}}" maxlength="191" autocomplete="off"
                               placeholder=""
                               class="layui-input">
                    </div>
                </div>
    ...
    <script type="text/javascript">
        layui.use(['element', 'form', 'jquery', 'layedit', 'laydate', 'systemGui'], function () {
            ...
            form.on('submit(formPOST)', function (data) {
                systemGui.createOrUpdate(data.field, 'POST','{{url('admin/tab')}}')
                return false;
            });
            //更新
            form.on('submit(formPUT)', function (data) {
                systemGui.createOrUpdate(data.field, 'PUT','{{url('admin/tab',$tab->id)}}')
